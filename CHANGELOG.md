...
- **Built-in Chat UI** (`GET /ui`): Full web interface served by the proxy with streaming chat, document upload, capability toggles (RAG, LLM2, Assistant), and dark theme.
...
- **LLM2 / Assistant Pipeline Integration**: LLM2 registry, seed transit, fractal analyzer, and resonance database wired into chat completions with per-request capability toggles (locked by default).
...
- `JanDocumentPlugin.spec`: Added all LLM2/Assistant pipeline `.py` files, `chat_ui.html`, `soul_registry_state.json`, `config.env.example`, `resource_monitor.py`, `ocr_processor.py`, `batch_processor.py` to `datas`. Added `speech_recognition` and `pyaudio` to `hiddenimports`.

...
export function TTSControls({ text, threadTitle, className = '' }: TTSControlsProps) {
  ...
  const handlePlay = async () => {
    ...
    try {
      ...
      toast.error(t('mobius:tts.failedToGenerate'))
    }
  }
      ...
      try {
        ...
        setTimeout(async () => {
          try {
            ...
            toast.success(t('mobius:tts.audioSaved'))
          } catch (err) {
            console.error('TTS save error:', err)
            toast.error(t('mobius:tts.failedToSave'))
          }
        }, 500)
      } catch (err) {
  ...
  return (
    <div className={`inline-flex items-center gap-1 ${className}`}>
      ...
      <button
        ...
        title={isPlaying ? t('mobius:tts.stop') : t('mobius:tts.readAloud')}
      >
        {isGenerating ? (
      ...
      <div className="relative">
        <button
          ...
          title={t('mobius:tts.selectVoice')}
        >
